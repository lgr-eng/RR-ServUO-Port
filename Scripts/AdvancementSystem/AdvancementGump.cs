...
namespace Server.Gumps
{
    ...
    {
        private const int MaxSkillCap = 100;
        private const int MaxTotalSkillCap = 30000;
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            if (skillIndexByButtonId.TryGetValue(info.ButtonID, out int skillIndex))
            {
                ...
                {
                    from.SendMessage($"{skill.Name} reached the {MaxSkillCap} soft cap. Hunt power scrolls to increase more.");
                }
                else
                {
                    from.SendMessage($"You need 1 Ancient Knowledge Scroll to increase the skill cap for {skill.Name} by 1.");
                }
            }
            ...
            switch (info.ButtonID)
            {
                case 2000: // Increase Total Skill Cap
                    if (from.SkillsCap < MaxTotalSkillCap && ConsumeScrolls(from, 2))
                    {
                        from.SkillsCap += 50;
                    ...
                    {
                        from.SendMessage($"You've reached the soft cap of {MaxTotalSkillCap / 10} skill points. Hunt for Royal Skil Charters to further increase the cap.");
                    }
                    else
                    {
                        from.SendMessage("You need 2 Ancient Knowledge Scrolls to increase your total skill cap by 5.");
                    }
                    break;

                case 2001: // Increase Total Stat Cap
                    if (from.StatCap < MaxStatCap && ConsumeScrolls(from, 3))
                    {
                        from.StatCap += 1;
                    ...
                    {
                        from.SendMessage($"You've reached the soft cap of {MaxStatCap} stat points. Visit the Royal Steward at the Throne Room and buy Royal Stat Charters to further increase the cap.");
                    }
                    else
                    {
                        from.SendMessage("You need 3 Ancient Knowledge Scrolls to increase your total stat cap by 1.");
                    }
                    break;

                case 2002: // Increase Max Followers
                    if (from.FollowersMax < MaxFollowers && ConsumeScrolls(from, 5))
                    {
                        from.FollowersMax += 1;
                    ...
                    {
                        from.SendMessage($"You've reached the soft cap of {MaxFollowers} Followers Slots. Hunt Pet Slot Deeds to fruther increase the cap.");
                    }
                    else
                    {
                        from.SendMessage("You need 5 Ancient Knowledge Scrolls to increase your Follower Slots by 1.");
                    }
                    break;
            }
        ...
        private void RecursiveSearchForScrolls(Container container, int amountNeeded, ref int found, List<Item> itemsToConsume)
        {
            ...
            {
                if (item is AncientKnowledgeScroll)
                {
